...
| **AI agents** | • **[`AGENTS.md`](AGENTS.md)** — tool-neutral guide (Claude Code, Cursor, Copilot, Codex, …) giving any agent the verify loop, conventions, and guardrails up front; a one-line `CLAUDE.md` bridges it for Claude Code, and `make new-app` keeps it in sync.<br>• **[`.mcp.json`](.mcp.json)** — a Claude Code agent gets build/test/run-sim + screenshots (XcodeBuildMCP) and live Apple-docs lookup (sosumi) out of the box.<br>• **`PreToolUse` guard hook** ([`.claude/`](.claude/)) — hard-blocks agents from editing `.pbxproj`/`.entitlements`/`.storyboard`/`.xib` (Claude Code; other agents honor it via `AGENTS.md`); humans editing in Xcode are unaffected. |
